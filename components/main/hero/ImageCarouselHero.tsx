...
export function ImageCarouselHero({
  ...
  useEffect(() => {
    anglesRef.current = images.map((_, i) => i * (360 / images.length))
  }, [images.length])

  // rAF animation loop — no React state, direct DOM updates
  useEffect(() => {
    ...
    const tick = (time: number) => {
      if (prevTimeRef.current) {
        ...
        anglesRef.current.forEach((angle, i) => {
          const newAngle = (angle + speed * dt) % 360
          ...
          if (el) {
            el.style.transform = `translate(${x}px, ${y}px) rotateX(${perspectiveY}deg) rotateY(${perspectiveX}deg) rotateZ(${images[i]?.rotation ?? 0}deg)`
          }
        })
      }
  ...
  return (
    <div className="relative w-full min-h-screen bg-gradient-to-b from-background via-background to-background overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen pt-24 px-4 sm:px-6 lg:px-8">
        {/* Carousel Container */}
        <div
          className="relative w-full max-w-6xl h-96 sm:h-[500px] mb-12 sm:mb-16 mt-8"
          ...
          <div className="absolute inset-0 flex items-center justify-center perspective">
            {images.map((image, index) => (
                <div
                  ...
                  className="absolute w-48 h-40 sm:w-60 sm:h-48 will-change-transform"
                  style={{
        ...
        <div className="relative z-20 text-center max-w-2xl mx-auto mb-12 sm:mb-16">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-handwriting font-bold text-foreground mb-4 sm:mb-6 text-balance leading-tight">
            {title}
          </h1>
